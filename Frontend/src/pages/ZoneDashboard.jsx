...
const talukaMap = {
  ...
  'Amravati': ['Achalpur', 'Amravati', 'Anjangaon', 'Bhatkuli', 'Chandur Bazar', 'Chandur Railway', 'Daryapur', 'Dharni', 'Morshi', 'Nandgaon Khandeshwar', 'Teosa', 'Warud'],
  'Aurangabad': ['Aurangabad', 'Gangapur', 'Kannad', 'Khuldabad', 'Paithan', 'Phulambri', 'Sillod', 'Soegaon', 'Vaijapur'],
  'Beed': ['Ambejogai', 'Ashti', 'Beed', 'Georai', 'Kaij', 'Majalgaon', 'Parli', 'Patoda', 'Shirur (Kasar)', 'Wadwani'],
  ...
  'Chandrapur': ['Ballarpur', 'Bhadravati', 'Bramhapuri', 'Chandrapur', 'Chimur', 'Gondpipri', 'Jiwati', 'Korpana', 'Mul', 'Nagbhir', 'Pombhurna', 'Rajura', 'Saoli', 'Sindewahi', 'Warora'],
  ...
  'Kolhapur': ['Ajra', 'Bhudargad', 'Chandgad', 'Gadhinglaj', 'Hatkanangale', 'Kagal', 'Karvir', 'Panhala', 'Radhanagari', 'Shahuwadi'],
  ...
  'Nagpur': ['Hingna', 'Kamptee', 'Katol', 'Kuhi', 'Mouda', 'Nagpur (Rural)', 'Nagpur (Urban)', 'Narkhed', 'Parseoni', 'Ramtek', 'Savner', 'Umred'],
  ...
  'Nashik': ['Baglan', 'Chandvad', 'Deola', 'Dindori', 'Igatpuri', 'Kalwan', 'Malegaon', 'Nandgaon', 'Nashik', 'Niphad', 'Peth', 'Peint', 'Sinnar', 'Surgana', 'Trimbakeshwar', 'Yeola'],
  'Osmanabad': ['Bhum', 'Kalamb', 'Lohara', 'Osmanabad', 'Paranda', 'Tuljapur', 'Vashi', 'Washi'],
  ...
  'Yavatmal': ['Arni', 'Babulgaon', 'Darwha', 'Digras', 'Ghatanji', 'Kalamb', 'Kelapur', 'Mahagaon', 'Maregaon', 'Ner', 'Pandharkawoda', 'Pusad', 'Ralegaon', 'Umarkhed', 'Wani', 'Yavatmal', 'Zari Jamni']
};

export default function ZoneDashboard() {
  const navigate = useNavigate();
  const { token, user } = useAuth();
  const [stats, setStats] = useState({ readers:{total:0,today:0}, ads:{total:0,pending:0,active:0}, notice:"Welcome to your Regional Coordinator Portal. Data is syncing..." });
  ...
  const [newsForm, setNewsForm] = useState({ name:'',phone:'',pinCode:'',category:'Matrimonial',durationDays:1,content:'',paymentAmount:500,photo:null,customCategory:'',photoError:false });
      ...
      .then(data => {
        if (data && data.ads) {
          ...
          setStats({
            ...
            notice: data.notice || "Welcome to your Regional Coordinator Portal."
          });
        }
        setLoading(false);
      })
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-gray-50 uppercase font-black text-navy animate-pulse">Loading Regional Dashboard Data...</div>;
  ...
  const handlePayment = async (method) => {
    ...
    try {
      if (paymentContext==='reader') {
        ...
        setNewsForm({name:'',phone:'',pinCode:'',category:'Matrimonial',durationDays:1,content:'',paymentAmount:500,photo:null,customCategory:'',photoError:false});setShowNewsModal(false);
      }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <main className="flex-1 px-4 py-8 max-w-6xl mx-auto w-full">
        <div className="mb-8">
          <h1 className="text-3xl font-black text-navy tracking-tight uppercase">Regional Dashboard</h1>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-8 gap-3 sm:gap-4 mb-8">
          ...
          <div className="bg-white p-4 sm:p-6 rounded-xl border border-steppergold/50 shadow-md flex flex-col hover:border-steppergold transition-all relative overflow-hidden">
             <div className="flex items-start justify-between mb-4">
                 <h4 className="text-[8px] sm:text-[9px] font-black text-navy uppercase tracking-tight leading-snug">District Reporters</h4>
                 <Users className="text-steppergold w-4 h-4 sm:w-5 sm:h-5 shrink-0 ml-1" />
             </div>
             ...
             <p className="text-[8px] sm:text-[10px] font-bold text-navy uppercase mt-auto pt-2">Region Level</p>
          </div>
          ...
          <div className="bg-white p-4 sm:p-6 rounded-xl border border-gray-200 shadow-sm flex flex-col hover:border-blue-500 transition-colors relative overflow-hidden">
             <div className="flex items-start justify-between mb-2">
                 <h4 className="text-[9px] sm:text-[10px] md:text-xs font-black text-gray-400 uppercase tracking-widest pr-4">Regional Readers</h4>
          ...
          <div className="bg-white p-4 sm:p-6 rounded-xl border border-steppergold/50 shadow-md flex flex-col hover:border-steppergold transition-colors relative overflow-hidden"><div className="absolute top-0 right-0 w-12 h-12 bg-steppergold/5 rounded-bl-[100%]"></div><div className="flex items-start justify-between mb-4"><h4 className="text-[9px] sm:text-[10px] md:text-xs font-black text-navy uppercase tracking-widest leading-tight pr-2">Subscribers Revenue</h4><span className="text-steppergold font-black shrink-0 text-sm">₹</span></div><p className="text-2xl sm:text-3xl xl:text-4xl font-black text-steppergold">₹{totalCommission.toLocaleString(undefined,{maximumFractionDigits:0})}</p><p className="text-[8px] sm:text-[10px] font-bold text-navy uppercase tracking-widest mt-auto pt-2">Earned via Subs</p></div>
          <div className="bg-white p-4 sm:p-6 rounded-xl border border-steppergold/50 shadow-md flex flex-col hover:border-steppergold transition-colors relative overflow-hidden"><div className="absolute top-0 right-0 w-12 h-12 bg-steppergold/5 rounded-bl-[100%]"></div><div className="flex items-start justify-between mb-4"><h4 className="text-[9px] sm:text-[10px] md:text-xs font-black text-navy uppercase tracking-widest leading-tight pr-2">ADVT Revenue</h4><Clock className="text-steppergold w-4 h-4 sm:w-5 sm:h-5 shrink-0"/></div><p className="text-2xl sm:text-3xl xl:text-4xl font-black text-steppergold">₹{newsCommission.toLocaleString(undefined,{maximumFractionDigits:0})}</p><p className="text-[8px] sm:text-[10px] font-bold text-navy uppercase tracking-widest mt-auto pt-2">Earned via Ads</p></div>
          <div className="bg-white p-4 sm:p-6 rounded-xl border border-gray-200 shadow-sm flex flex-col hover:border-blue-500 transition-colors relative overflow-hidden"><div className="flex items-start justify-between mb-4"><h4 className="text-[9px] sm:text-[10px] md:text-xs font-black text-gray-400 uppercase tracking-widest leading-tight pr-4">Regional Advt</h4><FileText className="text-orange-500 w-4 h-4 sm:w-5 sm:h-5 shrink-0"/></div><p className="text-2xl sm:text-3xl xl:text-4xl font-black text-gray-900">{allAdsCount}</p><p className="text-[8px] sm:text-[10px] font-bold text-gray-400 uppercase mt-auto pt-2">All time</p></div>
          <div className="bg-white p-4 sm:p-6 rounded-xl border border-gray-200 shadow-sm flex flex-col hover:border-blue-500 transition-colors relative overflow-hidden"><div className="flex items-start justify-between mb-4"><h4 className="text-[9px] sm:text-[10px] md:text-xs font-black text-gray-400 uppercase tracking-widest leading-tight pr-4">Active Advt</h4><CheckCircle className="text-green-500 w-4 h-4 sm:w-5 sm:h-5 shrink-0"/></div><p className="text-2xl sm:text-3xl xl:text-4xl font-black text-gray-900">{stats.ads.active}</p><p className="text-[8px] sm:text-[10px] font-bold text-gray-400 uppercase mt-auto pt-2">Live Now</p></div>
        </div>

        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 mt-8">
          <div className="xl:col-span-1 flex flex-col gap-3">
            <h3 className="text-xs font-black text-gray-800 uppercase tracking-widest mb-1 px-1">Regional Area Control</h3>
            ...
            <button onClick={() => { setEditingReaderId(null); setShowReaderModal(true); }} className="w-full p-4 bg-steppergold/10 hover:bg-steppergold/20 border-2 border-steppergold rounded-2xl flex items-center justify-between transition-all group active:scale-95">
              <div className="flex items-center gap-4">
                ...
                <div className="text-left"><p className="text-[10px] md:text-xs font-black text-navy uppercase">REGISTER NEW SUBSCRIBER</p><p className="text-[8px] md:text-[9px] font-bold text-gray-500 uppercase mt-0.5">Subscriber Management</p></div>
              </div>
            ...
            <button onClick={()=>setShowNewsModal(true)} className="w-full p-4 bg-gray-100 hover:bg-navy hover:text-white rounded-2xl flex items-center justify-between transition-all group active:scale-95">
              <div className="flex items-center gap-4">
                ...
                <div className="text-left"><p className="text-[10px] md:text-xs font-black uppercase">Post Regional Advt</p><p className="text-[8px] md:text-[9px] font-bold text-gray-500 uppercase mt-0.5">Publish Updates</p></div>
              </div>
              <ArrowRight className="group-hover:translate-x-1 transition-transform" size={16}/>
            </button>

            <button onClick={()=>setShowSimpleNewsModal(true)} className="w-full p-4 bg-navy/5 hover:bg-navy hover:text-white border-2 border-navy rounded-2xl flex items-center justify-between transition-all group active:scale-95 shadow-sm">
              <div className="flex items-center gap-4">
                <div className="bg-white p-2 rounded-lg shadow-sm group-hover:scale-110 transition-transform"><Newspaper className="text-navy w-5 h-5 group-hover:text-white transition-colors"/></div>
          ...
          <div className="xl:col-span-2"><div className="bg-white rounded-2xl shadow-sm border border-gray-200 h-full overflow-hidden flex flex-col"><div className="p-4 md:p-5 border-b border-gray-100 flex items-center justify-between bg-gray-50"><h3 className="text-[10px] md:text-xs font-black text-navy uppercase tracking-widest flex items-center gap-2"><Newspaper className="text-blue-500" size={16}/> Latest Regional Advt</h3><span className="text-[8px] md:text-[10px] font-bold text-gray-400 uppercase bg-white px-2 py-1 rounded-md shadow-sm border border-gray-100">Live Updates</span></div><div className="p-0 flex-1 overflow-y-auto max-h-[250px] md:max-h-[350px]">{latestNews.length===0?<div className="p-8 text-center text-gray-400 font-bold uppercase tracking-widest text-[10px]">No recent ads available.</div>:latestNews.map((news,idx)=><div key={idx} className="p-4 md:p-5 border-b border-gray-50 hover:bg-blue-50/30 transition-colors group cursor-pointer"><div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-1 gap-1"><h4 className="text-xs md:text-sm font-black text-gray-900 group-hover:text-blue-600 line-clamp-1">{news.title||'Latest Ad'}</h4><span className="text-[8px] font-bold text-gray-400 uppercase whitespace-nowrap">{new Date(news.createdAt||Date.now()).toLocaleDateString()}</span></div><p className="text-[10px] md:text-xs text-gray-600 line-clamp-2">{news.content||'View news payload...'}...</p></div>)}</div></div></div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 lg:grid-rows-2 xl:grid-cols-4 xl:grid-rows-1 gap-4 mt-8">
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden flex flex-col h-64 lg:h-[450px]">
             <div className="p-4 border-b border-navy/5 flex items-center justify-between bg-navy/5"><h3 className="text-[9px] font-black text-navy uppercase tracking-tight flex items-center gap-2"><Users className="text-navy" size={14}/> District Reporters</h3><span className="text-[8px] font-black text-navy/40 uppercase">Region Level</span></div>
             <div className="p-0 flex-1 overflow-y-auto">
                {reporters.filter(r => r.applying_for === 'district_coordinator').length === 0 ? <div className="p-8 text-center text-gray-300 font-bold text-[10px]">No District Staff</div> : reporters.filter(r => r.applying_for === 'district_coordinator').map((rep, idx) => (
                  <div key={rep._id} className="px-4 py-3 border-b border-gray-50 hover:bg-navy/5 transition-colors group">
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        <span className="w-7 h-7 rounded-full bg-navy/10 flex items-center justify-center text-[10px] font-black text-navy shrink-0">{idx + 1}</span>
                        <div className="min-w-0">
                          <h4 className="text-xs font-black text-navy truncate">{rep.name}</h4>
                        </div>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        <span className="text-[10px] text-gray-500 font-mono">{rep.mobile}</span>
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-orange-100 overflow-hidden flex flex-col h-64 lg:h-[450px]">
             <div className="p-4 border-b border-orange-50 flex items-center justify-between bg-orange-50/50"><h3 className="text-[9px] font-black text-orange-900 uppercase tracking-tight flex items-center gap-2"><Users className="text-orange-600" size={14}/> Taluka Reporters</h3><span className="text-[8px] font-black text-orange-400 uppercase">Area Management</span></div>
             <div className="p-0 flex-1 overflow-y-auto">
                {reporters.filter(r => r.applying_for === 'taluka_coordinator').length === 0 ? <div className="p-8 text-center text-gray-300 font-bold text-[10px]">No Taluka Staff</div> : reporters.filter(r => r.applying_for === 'taluka_coordinator').map((rep, idx) => (
                  <div key={rep._id} className="px-4 py-3 border-b border-gray-50 hover:bg-orange-50/10 transition-colors group">
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        <span className="w-7 h-7 rounded-full bg-orange-100 flex items-center justify-center text-[10px] font-black text-orange-600 shrink-0">{idx + 1}</span>
                        <div className="min-w-0">
                          <h4 className="text-xs font-black text-orange-900 truncate">{rep.name}</h4>
                        </div>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        <span className="text-[10px] text-gray-500 font-mono">{rep.mobile}</span>
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-blue-100 overflow-hidden flex flex-col h-64 lg:h-[450px]">
             <div className="p-4 border-b border-blue-50 flex items-center justify-between bg-blue-50/50"><h3 className="text-[9px] font-black text-blue-900 uppercase tracking-tight flex items-center gap-2"><Users className="text-blue-600" size={14}/> Village Reporters</h3><span className="text-[8px] font-black text-blue-400 uppercase">Field Force</span></div>
             <div className="p-0 flex-1 overflow-y-auto">
                {reporters.filter(r => r.applying_for === 'reporter' || !r.applying_for).length === 0 ? <div className="p-8 text-center text-gray-300 font-bold text-[10px]">No Village Staff</div> : reporters.filter(r => r.applying_for === 'reporter' || !r.applying_for).map((rep, idx) => (
                  <div key={rep._id} className="px-4 py-3 border-b border-gray-50 hover:bg-blue-50/10 transition-colors group">
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        <span className="w-7 h-7 rounded-full bg-blue-100 flex items-center justify-center text-[10px] font-black text-blue-600 shrink-0">{idx + 1}</span>
                        <div className="min-w-0">
                          <h4 className="text-xs font-black text-blue-900 truncate">{rep.name}</h4>
                        </div>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        <span className="text-[10px] text-gray-500 font-mono">{rep.mobile}</span>
      ...
      {/* Reader Modal */}
      {showReaderModal&&(<div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-navy/80 backdrop-blur-sm animate-in fade-in duration-300 overflow-y-auto"><div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-300 my-auto"><div className="p-6 md:p-8 bg-steppergold text-navy flex justify-between items-center gap-4"><div className="flex-1 text-left"><h2 className="text-lg md:text-2xl font-black uppercase tracking-tight text-left">{editingReaderId?'Update Subscriber':'REGISTER NEW SUBSCRIBER'}</h2><p className="text-[8px] md:text-[10px] text-navy/80 font-black uppercase tracking-widest mt-1 text-left">Regional Coordinator Portal</p></div><button onClick={()=>{setShowReaderModal(false);setEditingReaderId(null);setReaderForm({name:'',mobile:'',gender:'',education:'',address:'',district:'',taluka:'',village:'',pinCode:'',subscriptionPlan:'1_year',paymentAmount:1000});}} className="w-8 h-8 md:w-10 md:h-10 bg-navy/10 hover:bg-navy/20 rounded-full flex shrink-0 items-center justify-center"><span className="font-bold text-navy text-sm">✕</span></button></div>
        <div className="p-6 md:p-8 flex flex-col gap-4 bg-gray-50">
          <div className="flex flex-col md:grid md:grid-cols-2 gap-4">
            ...
            <div><label className="block text-[9px] md:text-[10px] font-black text-navy uppercase tracking-widest mb-2">District</label>
            ...
            <div><label className="block text-[9px] md:text-[10px] font-black text-navy uppercase tracking-widest mb-2">Taluka</label>
      ...
      {showNewsModal&&(<div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-navy/80 backdrop-blur-sm animate-in fade-in duration-300 overflow-y-auto"><div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-300 my-auto"><div className="p-6 md:p-8 bg-navy text-white flex justify-between items-center gap-4"><div className="flex-1"><h2 className="text-lg md:text-2xl font-black uppercase tracking-tight">Book ADVT</h2><p className="text-[8px] md:text-[10px] text-steppergold font-black uppercase tracking-widest mt-1">Regional Coordinator Portal</p></div><button onClick={()=>{setShowNewsModal(false);setNewsForm({name:'',phone:'',pinCode:'',category:'Matrimonial',durationDays:1,content:'',paymentAmount:500,photo:null,customCategory:'',photoError:false}); }} className="w-8 h-8 md:w-10 md:h-10 bg-white/10 hover:bg-white/20 rounded-full flex shrink-0 items-center justify-center"><span className="font-bold text-sm">✕</span></button></div>
        <div className="p-6 md:p-8 flex flex-col gap-4 bg-gray-50">
          <div className="flex flex-col md:grid md:grid-cols-2 gap-4">
          ...
          <div className="relative">
            <div className="flex justify-between items-center mb-2">
              <label className="block text-[9px] md:text-[10px] font-black text-navy uppercase tracking-widest">Ad / Advt Content</label>
            ...
            <textarea maxLength={2000} value={newsForm.content} onChange={e=>setNewsForm({...newsForm,content:e.target.value})} rows={4} className="w-full px-4 py-3 bg-white border border-gray-300 rounded-xl outline-none focus:border-navy font-bold text-xs resize-none" placeholder="Type the full ad content here..."/>
          </div>
          <div>
            ...
            <div className="flex items-center gap-3">
              <div className="relative flex-1">
                <input type="file" id="advt-photo-zone" accept="image/*" onChange={e=>{
                ...
                <label htmlFor="advt-photo-zone" className="flex items-center justify-center gap-2 w-full px-4 py-3 bg-white border-2 border-dashed border-gray-300 rounded-xl cursor-pointer hover:border-navy hover:bg-navy/5 transition-all">
                  <span className="text-[10px] font-black text-navy uppercase">{newsForm.photo ? 'Change Photo' : 'Choose File'}</span>
                </label>
              </div>
              {newsForm.photo && !newsForm.photoError && <div className="flex items-center justify-center w-8 h-8 bg-green-100 text-green-600 rounded-full"><span className="font-black">✓</span></div>}
            ...
            {newsForm.photo && !newsForm.photoError && (
              <div className="mt-2 flex items-center gap-2">
                <span className="text-[9px] font-black text-green-600 uppercase">✅ {newsForm.photo.name}</span>
              </div>
            )}
          </div>
                   ...
                   {[
                     ...
                     {days: 5, label: '5 Days', price: 2500}
                      ...
                      <label key={p.days} className={`flex items-center justify-between p-3 rounded-xl border-2 cursor-pointer transition-all ${newsForm.durationDays == p.days ? 'border-navy bg-navy/5' : 'border-gray-200 bg-white hover:border-blue-200'}`}>
                         <div className="flex items-center gap-3">
                            <input type="radio" name="advt_plan_z" className="w-4 h-4 text-navy border-gray-300 focus:ring-navy" 
      ...
      {showSimpleNewsModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-navy/80 backdrop-blur-sm animate-in fade-in duration-300 overflow-y-auto">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-300 my-auto">
            <div className="p-6 md:p-8 bg-navy text-white flex justify-between items-center gap-4">
              <div className="flex-1">
                <h2 className="text-lg md:text-2xl font-black uppercase tracking-tight">Submit News</h2>
                <p className="text-[8px] md:text-[10px] text-blue-200 font-black uppercase tracking-widest mt-1">Regional Coordinator Portal</p>
              </div>
              <button onClick={()=>{setShowSimpleNewsModal(false);setSimpleNewsForm({title:'',content:'',photo:null,photoError:false});}} className="w-8 h-8 md:w-10 md:h-10 bg-white/10 hover:bg-white/20 rounded-full flex shrink-0 items-center justify-center">
                <span className="font-bold text-sm">✕</span>
              </button>
            </div>
            <div className="p-6 md:p-8 flex flex-col gap-4 bg-gray-50">
              <div>
                <label className="block text-[9px] md:text-[10px] font-black text-navy uppercase tracking-widest mb-2">News Title</label>
                <input type="text" value={simpleNewsForm.title} onChange={e=>setSimpleNewsForm({...simpleNewsForm,title:e.target.value})} className="w-full px-4 py-3 bg-white border border-gray-300 rounded-xl outline-none focus:border-navy font-bold text-xs" placeholder="Enter news headline..."/>
              </div>
              <div>
                <div className="flex justify-between items-center mb-2">
                  <label className="block text-[9px] md:text-[10px] font-black text-navy uppercase tracking-widest">News Content</label>
                ...
                <textarea maxLength={2000} value={simpleNewsForm.content} onChange={e=>setSimpleNewsForm({...simpleNewsForm,content:e.target.value})} rows={5} className="w-full px-4 py-3 bg-white border border-gray-300 rounded-xl outline-none focus:border-navy font-bold text-xs resize-none" placeholder="Type the full news content here..."/>
              </div>
              <div>
                <label className="block text-[9px] md:text-[10px] font-black text-navy uppercase tracking-widest mb-2">Attach Photo (Max 5MB)</label>
                <div className="flex items-center gap-3">
                  <div className="relative flex-1">
                    <input type="file" id="news-photo-simple-zone" accept="image/*" onChange={e=>{
                      const file = e.target.files[0];
                      if(file && file.size > 5 * 1024 * 1024) {
                        e.target.value = '';
                    ...
                    <label htmlFor="news-photo-simple-zone" className="flex items-center justify-center gap-2 w-full px-4 py-3 bg-white border-2 border-dashed border-gray-300 rounded-xl cursor-pointer hover:border-navy hover:bg-navy/5 transition-all">
                      <span className="text-[10px] font-black text-navy uppercase">{simpleNewsForm.photo ? 'Change Photo' : 'Choose File'}</span>
                    </label>
                  </div>
                  {simpleNewsForm.photo && !simpleNewsForm.photoError && <div className="flex items-center justify-center w-8 h-8 bg-blue-100 text-navy rounded-full"><span className="font-black">✓</span></div>}
                ...
                {simpleNewsForm.photo && !simpleNewsForm.photoError && (
                  <div className="mt-2 p-2 bg-white rounded-xl border border-gray-200 flex flex-col gap-2">
              ...
              <button onClick={async()=>{
                ...
                try {
                  const res = await fetch('http://localhost:5000/api/news', {
                    ...
                    body: JSON.stringify({ name: user?.fullName || 'Regional Coordinator', title: simpleNewsForm.title, content: simpleNewsForm.content, category: 'News', paymentAmount: 0 })
                  });
                  const data = await res.json();
                  setLatestNews(prev => [data.news || { title: simpleNewsForm.title, content: simpleNewsForm.content, createdAt: new Date().toISOString() }, ...prev].slice(0,5));
                  setAllAdsCount(p=>p+1);
                  setShowSimpleNewsModal(false);
                  setSimpleNewsForm({title:'',content:'',photo:null,photoError:false});
                  alert('✅ News submitted successfully!');
                } catch(e) {
                  alert('News saved (Offline Mode).');
                  setShowSimpleNewsModal(false);
                }
          ...
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-300 my-auto">
            <div className="p-6 md:p-8 bg-black text-white flex justify-between items-center relative gap-4 text-left">
              <div className="flex-1 text-left">
                 ...
                 <p className="text-[8px] md:text-[10px] text-white/80 font-black uppercase tracking-widest mt-1 text-left uppercase">{editingReporterId ? 'Modify Details' : 'Regional Coordinator Portal'}</p>
              </div>
              <button 
