...
const ROLES = [
  ...
  { value: 'zone_coordinator', label: 'Zone Coordinator', badge: '🗺️', color: 'text-purple-600', bg: 'bg-purple-50' },
...
export default function Login() {
  ...
  const [role, setRole] = useState('village_coordinator');
  ...
  const handleLogin = async (e) => {
    ...
    if (success) {
      ...
      else if (role === 'village_coordinator' || role === 'village') navigate('/dashboard/village');
      else if (role === 'taluka_coordinator' || role === 'taluka') navigate('/dashboard/taluka');
      else if (role === 'district_coordinator' || role === 'district') navigate('/dashboard/district');
      else if (role === 'zone_coordinator' || role === 'zone') navigate('/dashboard/zone');
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#F8FAFC]">
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 bg-gradient-to-b from-white to-gray-50/50">
        <div className="w-full max-w-lg bg-white rounded-[3rem] shadow-[0_30px_90px_-15px_rgba(0,0,0,0.1)] border border-gray-100 overflow-hidden relative animate-in fade-in slide-in-from-bottom-6 duration-700">
          ...
          <div className="bg-navy p-12 text-center relative overflow-hidden">
             <div className="absolute top-0 left-0 w-full h-full opacity-10 bg-[radial-gradient(circle_at_50%_120%,rgba(255,183,0,1)_0%,transparent_80%)]"></div>
             <div className="w-20 h-20 bg-white/10 rounded-[2rem] flex items-center justify-center mx-auto mb-6 border border-white/20 shadow-2xl backdrop-blur-sm group hover:scale-110 transition-transform">
                <ShieldCheck className="w-10 h-10 text-steppergold" />
             </div>
             <h2 className="text-4xl font-black text-white uppercase tracking-tighter leading-none mb-4">Reporter Portal</h2>
             <div className="flex items-center justify-center gap-3">
                <span className="h-[2px] w-8 bg-steppergold/30 rounded-full"></span>
                <p className="text-steppergold text-[10px] font-black uppercase tracking-[0.4em]">Coordination & Auth</p>
                <span className="h-[2px] w-8 bg-steppergold/30 rounded-full"></span>
             </div>
          </div>

          <form onSubmit={handleLogin} className="p-12 space-y-10 relative">
            ...
            <button type="submit" disabled={loading}
              className="w-full bg-navy text-white h-20 rounded-[2.5rem] font-black uppercase tracking-[0.2em] text-[11px] hover:bg-black transition-all shadow-2xl shadow-navy/20 active:scale-[0.98] flex items-center justify-center gap-4 group overflow-hidden relative"
            >
              ...
              {loading ? (
                ...
                <>
                  Enter {currentRole?.label} Portal
                  <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
                </>
              )}
            </button>

            <div className="text-center pt-8 border-t border-gray-100 mt-2">
               <button onClick={() => navigate('/login')} className="flex items-center justify-center gap-2 mx-auto group">
                  ...
                  <div className="text-left">
                     <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest leading-none">Standard Authentication</p>
                     <p className="text-[11px] font-black text-navy uppercase tracking-tight group-hover:text-blue-600 transition-colors">Switch to Main Login</p>
                  </div>
               </button>
            </div>
          </form>
