...
const Header = () => (
  <div className="bg-white border-b-2 border-blue-800 flex justify-center items-center shadow-sm z-10 relative">
...
const ImageUpload = ({ label, name, value, onChange, height = "170px", subLabel, disabled = false, disabledMessage = "" }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-start shrink-0">
      <div
        className={`relative border-2 border-dashed ${value ? 'border-blue-500' : 'border-gray-400'} ${disabled ? 'bg-gray-200 cursor-not-allowed grayscale' : 'bg-gray-50 cursor-pointer hover:border-blue-500'} flex flex-col items-center justify-center overflow-hidden transition-colors group px-2 rounded-md`}
      ...
      >
        {value ? (
          ...
          <div className="flex flex-col items-center gap-3 text-center pointer-events-none p-2">
            <Upload className={`${disabled ? 'text-gray-400' : 'text-gray-400 group-hover:text-blue-500'} transition-colors`} size={32} />
...
const TermsModal = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-md z-[200] flex items-center justify-center p-2 md:p-4 animate-in fade-in duration-300 overflow-y-auto">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col border border-gray-200 animate-in zoom-in-95 duration-300">
        <div className="bg-blue-800 p-4 md:p-6 text-white flex justify-between items-center sticky top-0 z-10">
          <div>
            <h2 className="text-lg md:text-xl font-black uppercase tracking-tight">Terms & Conditions</h2>
            <p className="text-[10px] md:text-xs font-bold text-blue-200 uppercase tracking-widest opacity-80 mt-0.5">नियम आणि अटी</p>
          </div>
        ...
        <div className="p-4 md:p-8 overflow-y-auto bg-gray-50/50">
          <div className="space-y-6">
            {termsCount.map((num, i) => (
              <div key={num} className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex gap-4 items-start hover:border-blue-300 transition-colors group">
                <div className="bg-blue-800 text-white w-8 h-8 rounded-lg flex items-center justify-center font-bold text-sm shrink-0 shadow-md group-hover:scale-105 transition-transform">{num}</div>
                <div className="space-y-3">
                  <p className="text-xs md:text-sm text-gray-800 font-bold leading-relaxed">{termsEn[i]}</p>
                  <p className="text-xs md:text-sm text-blue-800 font-medium leading-relaxed bg-blue-50/50 p-2 rounded-lg border border-blue-100/50 italic">{termsMr[i]}</p>
                </div>
              </div>
            ))}
          </div>
          <div className="mt-8 text-center p-4">
            <button onClick={onClose} className="bg-blue-800 text-white px-8 py-3 rounded-2xl text-sm font-bold uppercase tracking-widest hover:bg-blue-900 transition-all shadow-lg hover:shadow-xl active:scale-95">Accept & Close</button>
          </div>
        </div>
      </div>
    </div>
...
const ReviewOverlay = ({ isOpen, onClose, formData, onEdit }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[250] flex flex-col items-center p-2 md:p-12 animate-in fade-in duration-500 overflow-hidden">
      <div className="w-full max-w-4xl bg-white shadow-xl overflow-hidden flex flex-col h-full border border-gray-100">
        <div className="bg-white p-6 border-b border-gray-100 flex justify-between items-center sticky top-0 z-10">
          <div>
            <h2 className="text-xl font-black text-blue-900 tracking-tight">Application Review</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-12 space-y-10 bg-white leading-relaxed">
          {sections.map(section => (
            <div key={section.id} className="space-y-4">
              <div className="flex items-center justify-between border-b border-gray-100 pb-2">
                <h3 className="text-sm font-black text-gray-800 uppercase tracking-wide">{section.title}:</h3>
                <button onClick={() => { onEdit(section.id); onClose(); }} className="text-[10px] font-black text-blue-600 hover:underline uppercase tracking-widest flex items-center gap-1">
                  [Edit Section]
                </button>
              </div>
                ...
                {section.fields.map(field => (
                  formData[field] && (
                    <li key={field} className="text-xs md:text-[13px] text-gray-600 flex gap-2">
                      <span className="shrink-0 text-blue-800 font-bold">•</span>
          ...
          <div className="pt-6 border-t border-gray-100">
            <div className="flex items-center justify-between border-b border-gray-100 pb-2 mb-4">
              ...
              <button onClick={() => { onEdit(6); onClose(); }} className="text-[10px] font-black text-blue-600 hover:underline uppercase tracking-widest flex items-center gap-1">
                [Edit Signature]
              </button>
            </div>
            <div className="flex justify-center bg-gray-50 p-6 rounded-xl border-2 border-dashed border-gray-200">
              {formData.sign_declaration ? (
                <div className="bg-white p-2 shadow-md border border-gray-100 flex flex-col items-center gap-2">
        ...
        <div className="p-8 bg-gray-50/50 border-t border-gray-100 text-center flex flex-col items-center gap-4">
          <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Verify and click finish to continue</p>
          <button onClick={onClose} className="bg-blue-800 text-white px-16 py-3 rounded-full text-xs font-black uppercase tracking-widest hover:bg-blue-900 transition-all shadow-lg active:scale-95">Finished Review</button>
        </div>
      </div>
    </div>
  );
...
const SuccessModal = ({ isOpen, onClose, formData, onDownloadPDF, onDownloadDoc }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[300] flex items-center justify-center p-2 md:p-4 animate-in fade-in duration-300">
      <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-300 border border-gray-100 flex flex-col max-h-[95vh]">
        <div className="bg-blue-800 p-6 text-center text-white relative shrink-0">
          <div className="bg-white text-green-500 rounded-full p-3 shadow-xl border-4 border-gray-50 flex items-center justify-center w-16 h-16 mx-auto mb-3">
            <ShieldCheck size={32} />
          </div>
          <h2 className="text-xl font-black uppercase tracking-tight">Success!</h2>
          <p className="text-blue-100 mt-1 text-[10px] font-bold uppercase tracking-widest opacity-80">Registration successful</p>
        </div>

        <div className="p-6 md:p-8 flex-1 overflow-y-auto space-y-6 bg-white">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 gap-2">
              <button
                ...
                className="w-full flex items-center justify-between gap-3 p-3 rounded-xl border-2 border-blue-600 bg-blue-50 text-blue-800 hover:bg-blue-100 transition-all group"
              >
                <div className="flex items-center gap-3">
                  <div className="bg-blue-600 text-white p-2 rounded-lg group-hover:scale-110 transition-transform"><FileText size={18} /></div>
                  <div className="text-left">
                    <p className="text-xs font-black uppercase">Application Form</p>
                    <p className="text-[9px] font-bold text-blue-400">PDF Document</p>
                  </div>
                </div>
                <Upload className="rotate-180 text-blue-300" size={16} />
              </button>
                ...
                {documents.map((doc) => (
                  formData[doc.field] && (
                    <button
                      ...
                      className="flex items-center gap-2 p-2 rounded-lg border border-gray-100 bg-gray-50 hover:bg-white hover:border-blue-200 transition-all group text-left"
                    >
                      <div className="bg-white text-gray-400 group-hover:text-blue-600 p-1.5 rounded transition-colors shadow-sm"><FileText size={14} /></div>
        ...
        <div className="p-6 bg-gray-50 border-t border-gray-100 flex gap-2">
          <button
            onClick={onClose}
            className="flex-1 bg-gray-800 text-white h-11 rounded-xl font-black uppercase tracking-widest text-[10px] hover:bg-black transition-all shadow-md active:scale-95 flex items-center justify-center gap-2"
          >
            Finish
          </button>
        </div>
...
const ForgotPasswordRoute = () => {
  ...
  return (
    <div className="flex-1 flex items-center justify-center p-4">
      <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-sm border border-gray-100 animate-in zoom-in-95 duration-300">
        <h2 className="text-2xl font-black text-blue-900 uppercase text-center mb-6">Reset Password</h2>
        {step === 1 ? (
          <div className="space-y-4">
            ...
            <button onClick={handleSendCode} disabled={loading || !email} className="w-full bg-blue-800 text-white h-12 rounded-md font-black uppercase tracking-widest text-sm hover:bg-blue-700 transition-all shadow-lg shadow-blue-200 disabled:opacity-50">
          ...
          <div className="space-y-4">
            ...
            <button onClick={handleReset} disabled={loading || !code || !newPassword} className="w-full bg-blue-800 text-white h-12 rounded-md font-black uppercase tracking-widest text-sm hover:bg-blue-700 transition-all shadow-lg shadow-blue-200 disabled:opacity-50">
              {loading ? "..." : "Reset Password"}
            </button>
          </div>
        )}
        <button onClick={() => navigate('/login')} className="mt-8 text-gray-400 font-bold uppercase text-[10px] hover:text-blue-800 transition-colors w-full text-center tracking-widest">Back to Login</button>
      </div>
    </div>
  );
};
...
const LoginRoute = ({ formData, setFormData, handleInputChange }) => {
  ...
  const onLogin = async () => {
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setFormData(prev => ({ ...prev, email: data.user.email, fullName: data.user.fullName }));
        navigate('/form');
      } else {
        alert(data.message || "Invalid Credentials! Please check your Email/Phone and Password.");
      }
    } catch (err) {
          ...
          <div className="space-y-4 flex-1">
            ...
            <div className="relative">
              <div className="flex justify-between items-center mb-2">
                ...
                <button onClick={() => navigate('/forgot-password')} className="text-xs md:text-sm text-blue-700 font-bold hover:underline">Forgot Password?</button>
              </div>
              <input
...
const RegistrationRoute = ({ formData, setFormData, handleInputChange, otpProps }) => {
  const navigate = useNavigate();
  const {
    otp, setOtp, isOtpSent, isVerified, loadingOtp, handleSendOtp, handleVerifyOtp, otpError, otpSuccess,
    isEmailVerified
  } = otpProps;
  const [showPassword, setShowPassword] = useState(false);
        ...
        <div className="p-4 md:p-6 space-y-4">
          ...
          {regStep === 1 && (
            <div className="space-y-3 animate-in fade-in slide-in-from-right-4 duration-500">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 border-b border-gray-100 pb-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 pb-2 border-b border-gray-100 items-end">
                ...
                <div className="flex flex-col items-start pb-[2px] w-full">
                  ...
                  <div className="bg-blue-50/50 border border-blue-100 flex items-center justify-center text-[9px] font-black text-blue-800 rounded shadow-inner h-8 px-4 w-full uppercase tracking-tighter">
          ...
          {regStep === 2 && (
            <div className="space-y-3 animate-in fade-in slide-in-from-right-4 duration-500">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 border-b border-gray-100 pb-3">
                  ...
                  {otpProps.isEmailOtpSent && !isEmailVerified && (
                    <div className="mt-1 flex gap-2 animate-in slide-in-from-top-2 duration-300">
                      <input
                        ...
                        className="border border-blue-400 p-1 rounded text-xs w-24 h-7 text-center shadow-sm focus:ring-1 focus:ring-blue-500 outline-none font-bold"
                      />
                      <button onClick={otpProps.handleVerifyEmailOtp} className="text-[10px] font-bold text-blue-800 uppercase bg-blue-50 px-2 rounded border border-blue-200 hover:bg-blue-100 h-7 transition-colors">Confirm</button>
                    </div>
                  )}
                  ...
                  {isOtpSent && !isVerified && (
                    <div className="mt-1 flex gap-2 animate-in slide-in-from-top-2 duration-300">
                      <input
                        ...
                        className="border border-blue-400 p-1 rounded text-xs w-24 h-7 text-center shadow-sm focus:ring-1 focus:ring-blue-500 outline-none font-bold"
                      />
                      <button onClick={handleVerifyOtp} className="text-[10px] font-bold text-blue-800 uppercase bg-blue-50 px-2 rounded border border-blue-200 hover:bg-blue-100 h-7 transition-colors">Confirm</button>
                    </div>
                  )}
...
const OfficialApplicationForm = ({ formData }) => {
  ...
  return (
    <div id="pdf-content" className="bg-white p-8 text-gray-800 font-sans leading-tight min-h-[1000px] w-full max-w-full">
      {/* Header */}
      <div className="flex justify-between items-center mb-8 border-b-2 border-blue-800 pb-6">
        <div>
          <h1 className="text-3xl font-black text-blue-900 tracking-tighter mb-0.5">BK TIMES</h1>
          <p className="text-[10px] font-bold text-gray-500 uppercase tracking-[0.25em]">– EK VILLAGE EK PATRAKAR MOHIM 2026 –</p>
        </div>
        <div className="text-right">
          <div className="bg-blue-800 text-white px-4 py-2 font-black text-[11px] uppercase tracking-widest inline-block rounded mb-1 shadow-sm">
            Official Enrollment
          </div>
      ...
      <div className="space-y-4">
        {sections.map((section, idx) => {
          ...
          return (
            <div key={idx} className="page-break-inside-avoid border border-blue-50/50 rounded-lg p-2.5 bg-gray-50/30">
              <h2 className="text-[11px] font-black text-blue-800 uppercase border-b border-blue-50 mb-2 pb-0.5 flex items-center gap-2">
                <div className="w-1.5 h-3 bg-blue-800"></div>
                {section.title}
              </h2>
              <div className="space-y-1.5 px-1">
                {rows.map((row, rIdx) => (
                  <div key={rIdx} className="flex gap-4">
                    {row.map((field, fIdx) => (
                      <div key={fIdx} className="flex-1 flex flex-col gap-0.5 text-[11px] border-b border-gray-100/50 pb-0.5">
      ...
      <div className="mt-6 pt-5 border-t-2 border-blue-800 flex flex-col page-break-inside-avoid">
        <div className="flex justify-between items-start gap-8">
          <div className="flex-1">
            <h3 className="text-[11px] font-black text-blue-800 uppercase mb-2 underline underline-offset-2">Declaration</h3>
            <p className="text-[10px] font-medium leading-relaxed italic text-gray-500 bg-gray-50 p-3 rounded-lg border border-gray-100">
              "I, <strong className="text-blue-900 uppercase not-italic">{formData.fullName || '__________________________'}</strong>, hereby declare that all the information provided above is complete and correct to the best of my knowledge. I understand and agree to follow all policies and codes of conduct as defined by BK TIMES."
            </p>
          ...
          <div className="text-center shrink-0">
            <div className="w-48 h-28 border-2 border-dashed border-blue-100 rounded-xl bg-gray-50 flex items-center justify-center p-2 mb-2 shadow-inner overflow-hidden">
              {formData.sign_declaration ? (
                <img
            ...
            <p className="text-[9px] font-black uppercase text-blue-800 tracking-widest leading-none">Handwritten Signature Only</p>
            <p className="text-[8px] font-bold text-gray-400 italic mt-0.5">Scanned Copy Verified</p>
          </div>
        </div>

        {/* Footer Branding */}
        <div className="mt-8 flex justify-between items-end border-t border-gray-100 opacity-60 pt-4">
          <p className="text-[8px] font-bold text-gray-400 uppercase tracking-widest">BK TIMES ENROLLMENT PORTAL © 2026</p>
          <div className="flex items-center gap-1.5 grayscale opacity-50">
            <div className="w-4 h-4 rounded-full bg-blue-800 flex items-center justify-center text-[7px] text-white font-black">BK</div>
            <span className="text-[8px] font-black text-blue-900 tracking-tighter uppercase">OFFICIAL SYSTEM RECORD</span>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="animate-in fade-in duration-500">
      ...
      {(showRegion || showDistrict) && (
        <div className={`grid gap-4 bg-blue-50 p-4 rounded-lg border border-blue-100 mb-2 ${showRegion && showDistrict ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-1 justify-items-center'}`}>
          {showRegion && (
      ...
      {(showTaluka || showVillage) && (
        <div className={`grid gap-4 bg-blue-50 p-4 rounded-lg border border-blue-100 mb-4 ${showTaluka && showVillage ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-1 justify-items-center'}`}>
          {showTaluka && (
            <div className="w-full max-w-md">
  ...
  <div className="animate-in fade-in duration-500">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 border-b border-gray-100 pb-3">
      ...
      <div className="lg:col-span-2 flex flex-col justify-end pb-[2px]">
        ...
        <div className="bg-gray-50 border border-gray-300 flex items-center justify-center text-[10px] font-black text-black rounded shadow-inner h-8 md:h-10 w-full uppercase tracking-tight">
          {(() => {
            const ageComp = calculateAge(formData.dob);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        ...
        <div className="bg-white rounded border border-blue-100 shadow-sm overflow-hidden">
          <div className="bg-blue-800 px-3 py-1">
            <p className="text-white text-[10px] md:text-xs font-bold uppercase tracking-wide"> Highest Educational Proof</p>
          </div>
          <div className="p-2 flex flex-col gap-2">
            <div className="flex flex-col gap-1">
              <label className="text-[10px] font-bold text-gray-500 uppercase tracking-tight">Qualification Type</label>
              <select name="highest_qualification_type" className="w-full border border-gray-300 p-1.5 rounded text-[10px] md:text-xs outline-none focus:ring-1 focus:ring-blue-500 bg-white" value={formData.highest_qualification_type || ''} onChange={handleInputChange}>
            ...
            <div className="flex items-center justify-between border-t border-gray-50 pt-2 mt-1">
              <p className="text-[10px] md:text-xs font-semibold text-gray-600">Marksheet / Certificate</p>
              <label className={`flex items-center gap-1 px-4 py-1.5 rounded cursor-pointer text-[9px] md:text-[10px] font-bold uppercase ${formData.doc_education ? 'bg-green-100 text-green-700 border border-green-300' : 'bg-blue-50 text-blue-600 border border-dashed border-blue-300 hover:bg-blue-100'}`}>
                <Upload size={12} />
        ...
        <div className="bg-white rounded border border-blue-100 shadow-sm overflow-hidden">
          ...
          <div className="p-2">
            <div className="flex items-center gap-2">
              <input name="police_verification" className="flex-1 border border-gray-300 p-1.5 rounded text-[10px] md:text-xs outline-none focus:ring-1 focus:ring-blue-500" value={formData.police_verification || ''} onChange={handleInputChange} placeholder="Enter Police Verification Number (Digits Only)" />
...
const Step6 = ({ formData, handleInputChange, handleFileChange, isPaid, handlePayment }) => {
  ...
  return (
    <div className="animate-in fade-in duration-500">
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-3">
        <div className="space-y-4 text-xs md:text-sm text-gray-800 font-medium leading-relaxed">
          <div className="flex flex-col gap-3">
            <p className="flex gap-2 border-b border-gray-100 pb-2">
              <span className="font-bold shrink-0">I,</span>
              <span>{formData.applying_for} of <strong className="text-blue-800 underline underline-offset-2">{formData.region || '_________'}</strong> (Area) of BK Times, Nashik.</span>
            </p>
            <div className="flex flex-col gap-2 mt-2">
              <p className="text-[10px] md:text-xs text-gray-500 font-bold uppercase tracking-wider flex items-center gap-2">
                <Info size={14} className="text-blue-600" /> Before submitting, please read our policies:
              </p>
              <button
                type="button"
                onClick={() => setShowTerms(true)}
                className={`text-blue-700 font-black uppercase text-[11px] md:text-xs underline hover:text-blue-900 transition-colors flex items-center gap-1 group w-fit ${termsRead ? 'text-green-600' : ''}`}
              >
                {termsRead ? 'Terms & Conditions Read ✓' : 'Read Terms & Conditions (नियम आणि अटी वाचा)'}
      ...
      <div className="mt-6 pt-5 border-t border-gray-200 flex flex-col items-center sm:items-end p-2 md:p-4">
        <div className={`p-4 border-2 rounded-xl shadow-inner transition-all duration-500 overflow-hidden relative ${termsRead ? 'bg-white border-blue-100 ring-4 ring-blue-50/50' : 'bg-gray-100 border-gray-200 grayscale opacity-50'}`}>
          <div className="flex flex-col items-center gap-1.5 mb-3">
            <h4 className="text-[10px] font-black text-blue-800 uppercase tracking-widest flex items-center gap-1">
          ...
          <div
            className={`w-[260px] h-[100px] border-2 border-dashed rounded-lg bg-gray-50 flex items-center justify-center p-2 relative group transition-all ${formData.sign_declaration ? 'border-green-500 bg-green-50/20' : 'border-gray-300 hover:border-blue-400'}`}
            onClick={() => termsRead && document.getElementById('signature-uploader').click()}
          >
            {formData.sign_declaration ? (
              <div className="relative w-full h-full flex flex-col items-center justify-center bg-white rounded-md shadow-sm border border-green-100 overflow-hidden">
                <img
                  src={formData.sign_declaration}
                  alt="Signature"
                ...
                if (file) {
                  ...
                  reader.onloadend = () => {
                    ...
                    img.onload = () => {
                      ...
                      if (ar < 0.7 || ar > 10) {
                        alert("❌ REJECTED: INVALID FORMAT\n\nThe image dimensions do not match a signature scan.");
                        target.value = '';
                        return;
                      }
                      ...
                      const canvas = document.createElement('canvas');
                      const ctx = canvas.getContext('2d');
                      canvas.width = 100; canvas.height = Math.round(100 / ar);
                      ...
                      for (let i = 0; i < imageData.length; i += 4) {
                        ...
                        if (maxDiff > 30) vibrant++;
                        const avg = (r + g + b) / 3;
                        if (avg < 150) darkPixels++;
                      }

                      const colorPercent = (vibrant / (canvas.width * canvas.height)) * 100;
                      const inkPercent = (darkPixels / (canvas.width * canvas.height)) * 100;

                      if (colorPercent > 4) {
                        alert("❌ REJECTED: Only monochromatic handwritten signatures are allowed. Logo or colored image detected.");
                        target.value = '';
                        return;
                      }
                      if (inkPercent > 8) {
                        alert("❌ REJECTED: Logo/Text block detected. A handwritten signature is made of thin pen lines. Your upload is too dense/thick.");
                        target.value = '';
                        return;
                      }
                      if (inkPercent < 0.2) {
                        alert("❌ REJECTED: Signature not detected. Please use dark ink.");
                        target.value = '';
                        return;
                      }
      ...
      {fees && fees.total > 0 && (
        <div className="mt-4 pt-4 border-t border-gray-200 animate-in fade-in slide-in-from-top-4 duration-700 w-full flex justify-center sm:justify-end">
          <div className="bg-blue-50/50 border border-blue-100 p-4 rounded-xl shadow-inner w-full max-w-md">
            <h3 className="text-[10px] font-black text-blue-900 uppercase mb-3 flex items-center gap-2 tracking-widest">
            ...
            <div className="space-y-1.5 mb-4">
              ...
              <div className="h-px bg-blue-200 my-1.5 opacity-50"></div>
              <div className="flex justify-between text-xs font-black text-blue-900 uppercase tracking-tight">
            ...
            {isPaid ? (
              ...
              <>
                <button
                  ...
                  className="w-full bg-blue-800 text-white h-10 rounded-lg font-black uppercase tracking-widest text-[10px] hover:bg-blue-900 shadow-lg transition-all flex items-center justify-center gap-2 group disabled:opacity-50 disabled:grayscale mb-4"
                >
                ...
                <div className="flex flex-col gap-2 border-t border-blue-100 pt-4">
                  <p className="text-[9px] font-black text-blue-400 uppercase text-center tracking-tighter mb-1">— Faster Pay via Mobile Apps —</p>
                  <div className="grid grid-cols-3 gap-2">
                    <button 
                      onClick={() => handlePayment('phonepe')}
                      disabled={!formData.sign_declaration}
                      className="flex flex-col items-center justify-center gap-1 bg-white border border-gray-200 p-2 py-3 rounded-xl hover:bg-purple-50 hover:border-purple-200 transition-all shadow-sm active:scale-95 disabled:opacity-50 group"
                    >
                    ...
                    <button 
                      ...
                      disabled={!formData.sign_declaration}
                      className="flex flex-col items-center justify-center gap-1 bg-white border border-gray-200 p-2 py-3 rounded-xl hover:bg-blue-50 hover:border-blue-200 transition-all shadow-sm active:scale-95 disabled:opacity-50 group"
                    >
                      <div className="w-8 h-8 rounded-full bg-blue-600 flex items-center justify-center text-white font-black text-[10px] shadow-sm group-hover:scale-110 transition-transform">G</div>
                      <span className="text-[8px] font-bold text-gray-500 uppercase tracking-tighter">G-Pay</span>
                    </button>
                    <button 
                      onClick={() => handlePayment('paytm')}
                      disabled={!formData.sign_declaration}
                      className="flex flex-col items-center justify-center gap-1 bg-white border border-gray-200 p-2 py-3 rounded-xl hover:bg-sky-50 hover:border-sky-200 transition-all shadow-sm active:scale-95 disabled:opacity-50 group"
                    >
...
const FormRoute = ({ formData, setFormData, handleInputChange, handleFileChange }) => {
  ...
  const steps = [
    ...
    { title: "Finalize", component: <Step6 formData={formData} handleInputChange={handleInputChange} handleFileChange={handleFileChange} isPaid={isPaid} handlePayment={handlePayment} /> }
  ];
  ...
  return (
    <div className="mx-auto w-full md:max-w-5xl md:my-6 bg-white shadow-2xl rounded-2xl overflow-hidden flex-1 flex flex-col h-fit animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="p-4 md:p-8 pt-4 md:pt-6">
        <div className="print:hidden">
          ...
          <div className="mt-8 flex justify-center gap-4 md:gap-8 pt-6 border-t pb-2">
            ...
            <div className="flex gap-2">
              {currentStep === 6 && <button onClick={() => setShowReview(true)} className="bg-gray-800 text-white px-4 md:px-6 py-2 rounded-full text-xs md:text-sm font-bold uppercase flex gap-1 items-center hover:bg-black transition-all shadow-md"><Search size={16} /> Review Form</button>}
        ...
        <div id="capture-pdf" ref={pdfRef} className="hidden print:block bg-white"><OfficialApplicationForm formData={formData} /></div>
      </div>
...
const App = () => {
  ...
  const otpProps = {
    otp, setOtp, isOtpSent, isVerified, loadingOtp, handleSendOtp, handleVerifyOtp, otpError, otpSuccess,
    emailOtp, setEmailOtp, isEmailOtpSent, isEmailVerified, loadingEmailOtp, handleSendEmailOtp, handleVerifyEmailOtp, emailOtpError, emailOtpSuccess
  };
      ...
      <div className="min-h-screen bg-gray-200 font-sans flex flex-col text-xs">
        ...
        <footer className="bg-blue-900 text-white p-4 text-center mt-auto">
          ...
          <div className="text-[10px] tracking-[0.2em] font-light opacity-80">
            BK TIMES REGISTRATION PORTAL © {new Date().getFullYear()}
          </div>
        </footer>
      </div>
    </Router>
